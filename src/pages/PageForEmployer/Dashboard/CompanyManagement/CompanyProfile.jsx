...
const CompanyProfile = () => {
  ...
  const [bannerUrl, setBannerUrl] = useState(null); // New state for banner URL
const [uploadBannerLoading, setUploadBannerLoading] = useState(false); // New state for banner upload loading
  ...
  useEffect(() => {
    const fetchEmployerData = async () => {
      try {
        ...
        setBannerUrl(data.companyBanner);
        console.log(data);
  ...
  const handleUpdate = async (values) => {
    try {
      ...
      const updateData = {
        ...
        companyBanner: bannerUrl || employer.companyBanner || null,
      ...
      setBannerUrl(refreshedData.companyBanner);
    } catch (error) {
        ...
        {isEditing && activeTab === "1" ? (
          <Form form={form} layout="vertical" onFinish={handleUpdate}>
            <Row gutter={[24, 24]}>
              ...
              <Col span={24}>
                <Form.Item
                  name="companyBanner" // Optional: for form instance to track, but visual state is bannerUrl
                  label="Banner công ty"
                  tooltip="Upload banner công ty (định dạng: JPG, PNG). Khuyến nghị kích thước 1200x400px."
                >
                  <Upload
                    name="banner"
                    listType="picture-card"
                    className="banner-uploader" // You might want to style this differently
                    showUploadList={false}
                    customRequest={async ({ file, onSuccess, onError }) => {
                      setUploadBannerLoading(true);
                      try {
                        const token = localStorage.getItem("token");
                        // Assuming uploadService.uploadAvatar can be used or you have a generic uploadImage
                        // If you have a specific service for banner, use that:
                        // const url = await uploadService.uploadBanner(file, token);
                        const url = await uploadService.uploadAvatar(file, token); // Or a more generic uploadService.uploadImage
                        setBannerUrl(url);
                        // form.setFieldsValue({ companyBanner: url }); // Update form if needed
                        message.success("Upload banner thành công");
                        onSuccess(); // Ant Design's onSuccess
                      } catch (error) {
                        message.error("Lỗi khi upload banner: " + (error.message || "Upload thất bại"));
                        onError(error); // Ant Design's onError
                      } finally {
                    ...
                    beforeUpload={(file) => {
                      ...
                      const isLt5M = file.size / 1024 / 1024 < 5; // Banners can be larger, adjust as needed (e.g., 5MB)
                      if (!isLt5M) {
                        message.error("Ảnh phải nhỏ hơn 5MB!");
                        return Upload.LIST_IGNORE;
                      }
                      return true; // Proceed with customRequest
                    }}
                  >
                    {bannerUrl ? (
                      <img
                        ...
                        style={{
                          width: "100%",
                          height: "100%",
                          objectFit: "contain", // Use 'contain' or 'cover' as preferred for banner preview
                        }}
                      />
                    ) : (
                      <div>
                        {uploadBannerLoading ? <LoadingOutlined /> : <PlusOutlined />}
          ...
          >
            <TabPane tab="Thông tin cơ bản" key="1">
              <Descriptions bordered column={2}>
                ...
                <Descriptions.Item label="Banner công ty" span={employer.companyBanner ? 3 : 1}>
                  {employer.companyBanner ? (
                    <img
                      src={employer.companyBanner}
                      alt="company banner"
                      style={{ width: "100%", maxHeight: "250px", objectFit: "contain", borderRadius: "4px" }}
                    />
                  ) : (
                    <span style={{ color: "#999" }}>Chưa có banner</span>
                  )}
                </Descriptions.Item>
